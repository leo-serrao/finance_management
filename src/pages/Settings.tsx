...
export default function Settings() {
  ...
  async function handleSaveProfile(e: React.FormEvent) {
    ...
    try {
      ...
      setToast({ message: 'Perfil salvo' })
    } catch (err) {
      console.error('Failed to save profile', err)
      setToast({ message: 'Erro ao salvar perfil' })
    }
  }
  ...
  async function handleDeleteFixed(id: string) {
    ...
    try {
      ...
      // allow undo
      setToast({ message: 'Gasto fixo excluído', actionLabel: 'Desfazer', onAction: async () => {
        try {
      ...
      <section className="max-w-lg">
        ...
        {editing && (
          <Modal title="Editar gasto fixo" onClose={() => setEditing(null)}>
            <form onSubmit={async (e) => { e.preventDefault(); if (!user) return; try { await updateFixedExpenseInUser(user.uid, editing.id, { name: editing.name, amount: editing.amount }); setEditing(null); setToast({ message: 'Gasto atualizado' }) } catch (err) { console.error(err); setToast({ message: 'Erro ao atualizar' }) } }} className="space-y-2">
